...
def _choose_next_idx_with_ptrue_tie(
    *,remaining: Sequence[int],
    ...
    flip_to_true: bool,tie_abs_gap: float,tie_rel_gap: float,tie_max_candidates: int,
):
    if not remaining:
    ...
    for idx in scored:
        ...
        if gap_abs <= tie_abs_gap or gap_rel <= tie_rel_gap:
            bucket.append(idx)
    ...
    while True:
        ...
        if enable_ptrue_tie and scores_vec is not None:
            next_idx, tie_info = _choose_next_idx_with_ptrue_tie(remaining=remaining,scores_vec=scores_vec,
                ...
                flip_to_true=p_true_flipping,tie_abs_gap=tie_abs_gap,tie_rel_gap=tie_rel_gap,tie_max_candidates=tie_max_candidates,)
        else:
